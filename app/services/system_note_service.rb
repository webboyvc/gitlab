...
module SystemNoteService
  ...
  def add_commits(noteable, project, author, new_commits, existing_commits = [], oldrev = nil)
    ...
    text_parts = ["增加了 #{commits_text}"]
    text_parts << commits_list(noteable, new_commits, existing_commits, oldrev)
    text_parts << "[与上一版本比较差异](#{diff_comparison_url(noteable, project, oldrev)})"
  ...
  def change_label(noteable, project, author, added_labels, removed_labels)
    ...
    if added_labels.present?
      text_parts << "增加 #{added_labels}"
      text_parts << '并' if removed_labels.present?
    end

    if removed_labels.present?
      text_parts << "删除 #{removed_labels}"
    end
